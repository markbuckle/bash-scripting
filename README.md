...
FIRST_NAME=Mark LAST_NAME=Buckle echo $FIRST_NAME $LAST_NAME

echo What is your first name? read FIRST_NAME echo What is your last name? read
LAST_NAME
...
Arguments that are at a specific position, counting from one (0) onwards.
Position 0 is reserved for the shell itself.
...
Piping allows you to perform a specific action on the output of a command. Let's
say the output of a command is very long and you want to filter something out.
Piping allows you to filter out what you want. "|" is the pipe symbol.
...
Example: To print Hello World! into a file called hello.txt:

```sh
echo Hello World! > hello.txt
```

Note that if you use the > symbol again, it will overwrite the previous output
with the new output rather than adding on to it. This is where appending comes
in place:
...
"$?" returns the value of the exit code on the last executed command. Exit code
"0" means the command was executed without any issues. Exit code "1" means the
...
If the option matches the case, then we will perform the action bound to that
option.
...
When you define variables, they are available to the entire script by default.
To avoid overwriting global variables you need to use local variables.
...
Allows you to filter file contents or the output of a command in such a way that
we can print out the most essential parts and get the output the way we like it.
...
Create a simple csv file and enter "one,two,three". Make sure to include commas
instead of spaces:
...
's' stands for substitute and is in the mode place. 'g' stands for globally and
is in the location place.
...
Exit the file with 'Ctrl + X', 'Y', 'Enter' to save the file.

To give the file permission to be executed:
...
Example: Within your bestdayever.sh file:
...
"read" is used to get user input. "name" is stored inside the variable $name
...
$1 is a positional parameter aka argument that means the first parameter after
./bestdayever.sh will be the variable input.
...
To figure out who you are logged in as:

```sh
whoami
```

Where am I in the fiel system:
...
If you want a variable to equal the output of a command:
